...
class LiveFsPlayer:
	...
	def play(self, midi_path: str, loop: bool = True):
		"""
		Preloads MIDI, starts playback
		"""
		# preload MIDI events ONCE (gapless looping key)
	...
	def _preload_events(self, midi_path):
		...
		res = []
		# filter out all the PC messages and tempo messages
		for msg in mid:
			if msg.type != "program_change" and msg.type != "set_tempo":
				tp = (msg.time, msg)
				res.append (tp)
		return res
